...
const DefaultPartialChargesPropertyParams = PD.clone(
...
function getData(
  model: Model
): CustomProperty.Data<SBNcbrPartialChargeData | undefined> {
  ...
  if (!SbNcbrPartialChargesPropertyProvider.isApplicable(model)) {
    ...
    const typeIdToResidueToCharge = getTypeIdToResidueIdToCharge(
      model,
      typeIdToAtomIdToCharge
    );
    const maxAbsoluteAtomCharges = getMaxAbsoluteCharges(
      typeIdToAtomIdToCharge
    );
    const maxAbsoluteResidueCharges = getMaxAbsoluteCharges(
...
function getTypeIdToMethod(model: Model) {
  ...
  const rowCount =
    sourceData.data.frame.categories.sb_ncbr_partial_atomic_charges_meta
      .rowCount;
  const typeIds =
    sourceData.data.frame.categories.sb_ncbr_partial_atomic_charges_meta.getField(
      "id"
    );
  const methods =
    sourceData.data.frame.categories.sb_ncbr_partial_atomic_charges_meta.getField(
...
function getTypeIdToAtomIdToCharge(
  ...
  const atomIdToCharge: SBNcbrPartialChargeData["typeIdToAtomIdToCharge"] =
    new Map();
  ...
  const typeIds =
    sourceData.data.frame.categories.sb_ncbr_partial_atomic_charges.getField(
      "type_id"
    );
  const atomIds =
    sourceData.data.frame.categories.sb_ncbr_partial_atomic_charges.getField(
...
function getTypeIdToResidueIdToCharge(
  ...
  const residueToCharge: SBNcbrPartialChargeData["typeIdToResidueToCharge"] =
    new Map();
...
> = CustomModelProperty.createProvider({
  ...
  obtain: (_ctx: CustomProperty.Context, model: Model) =>
    Promise.resolve(getData(model)),
});
